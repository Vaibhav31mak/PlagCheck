...
bool ForwardRefMaker::isEqualMinRun(size_t i, size_t j) {
	assert((i + PlagCheck::MinRunSize - 1 < _vector.size()) 
		&& (j + PlagCheck::MinRunSize - 1 < _vector.size()));

	size_t offset = 0;
	while (offset < PlagCheck::MinRunSize) {
...
#define	Left_Circular_32(i, s)	(((i) << (s)) | ((i) >> (32-(s))))
	...
	int oldestTokenSteppedLength = (shift * (PlagCheck::MinRunSize - 1)) % 32;
	for (const auto& text: _texts) {
		uint32_t hashValue = 0;
		for (size_t pos = text.begin(); pos < text.end(); ++pos) {
			if (pos - text.begin() + 1 > PlagCheck::MinRunSize) {
				int oldestTokenValue = _vector[pos - PlagCheck::MinRunSize].toInt();
			...
			size_t runStartPos = pos - PlagCheck::MinRunSize + 1;
	...
	for (size_t i = 0; i + PlagCheck::MinRunSize < _vector_length; ++i) {
