...
template<>
const struct PlagCheck::Idf LangBase<LangPython>::Idf_PreprocessCommands[] = {
    {"define",		META('d')},
};

template<>
const struct PlagCheck::Idf LangBase<LangPython>::Idf_Keywords[] = {
    {"and",          NORM('a')},
    {"as",           NORM('A')},
    {"assert",       CTRL('A')},
    {"async",        META('a')},
...
size_t LangPython::getLengthOfKeywordsList() {
    return sizeof(LangPython::Idf_Keywords) / sizeof(PlagCheck::Idf);
}
