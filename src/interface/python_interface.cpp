...
void callTokenizer(
    PlagCheck::Tokenizer& tokenizer, 
    std::vector<PlagCheck::Text>& texts, 
    const char* code
) {
...
static PyObject* compare(PyObject* self, PyObject* args) {
    ...
    PlagCheck::Lang* language = nullptr;

    std::vector<PlagCheck::Token> tokenVector = {
        PlagCheck::Token::None
    };
    std::vector<PlagCheck::Text> texts;
    ...
    if (strcmp(lang_type_string, "C") == 0) {
        language = PlagCheck::LangCpp::getInstance();
    }
    else if (strcmp(lang_type_string, "C++") == 0) {
        language = PlagCheck::LangCpp::getInstance();
    }
    else if (strcmp(lang_type_string, "Python3") == 0) {
        language = PlagCheck::LangPython::getInstance();
    }
    else {
        PyErr_SetString(PyExc_ValueError, 
    ...
    PlagCheck::Tokenizer tokenizer(tokenVector, language);
    ...
    PlagCheck::Comparer comparer(tokenVector, texts, language);
    return PyFloat_FromDouble(comparer.run());
}

static PyMethodDef MyMethods[] = {
    {"compare", compare, METH_VARARGS, "PlagCheck interface"},
    {NULL, NULL, 0, NULL}
};
...
PyMODINIT_FUNC PyInit_libPlagCheck(void) {
